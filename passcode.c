...
GET_KEY:
  ...
  delay_ms(500);
  while(GetKey() != 0xFF); // wait key to be released
  ...
  // if ENTER key pressed
    ...
    // if user code equals to system code
  ...
  // if a digit pressed
    ...
    if(usr_code_idx < sizeof(usr_code) - 1)
      {
      usr_code[usr_code_idx] = k + 0x30; // save the ascii code of digit
      usr_code_idx++;
      lcd_putchar('*');
      //lcd_puts(usr_code);
      }
